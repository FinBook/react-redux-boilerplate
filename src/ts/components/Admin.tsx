...
export default class Admin extends React.PureComponent<IAdminProps, IState> {
	private async init() {
		...
		const takerWETHApprovalTxHash = await assetsUtil.contractWrappers.erc20Token.setUnlimitedProxyAllowanceAsync(etherTokenAddress, taker);
		...
		const takerWETHDepositTxHash = await assetsUtil.contractWrappers.etherToken.depositAsync(etherTokenAddress, Web3Wrapper.toBaseUnitAmount(new BigNumber(TAKER_ETH_DEPOSIT), 18), taker);
	...
	private async sendingMessage() {
		...
		const makerAssetAmount = Web3Wrapper.toBaseUnitAmount(new BigNumber(Number(Math.random() * 10 || 5).toFixed(3)), 18);
		// the amount the maker wants of taker asset
		const takerAssetAmount = Web3Wrapper.toBaseUnitAmount(new BigNumber(Number(Math.random() || 5).toFixed(3)), 18);

		// Create the order
		const order: Order = { exchangeAddress, makerAddress: maker, takerAddress: taker, senderAddress: maker, feeRecipientAddress: taker, expirationTimeSeconds: randomExpiration, salt: generatePseudoRandomSalt(), makerAssetAmount, takerAssetAmount, makerAssetData, takerAssetData, makerFee: new BigNumber(0), takerFee: new BigNumber(0) };
		...
		const msg = { type: WsChannelMessageTypes.Add, channel: { name: WsChannelName.Orders, marketId: 'ZRX-ETH' }, requestId: Date.now(), payload: { order: signedOrder, orderHash: orderHashHex } };
	...
	public componentDidMount() {
		ws.onmessage = (event: any) => {
			...
			this.setState({ result: event.data, orderBook: msg });
	...
	constructor(props: IAdminProps) {
		super(props);
		this.state = { result: '', orderBook: { type: '', channel: { name: '', marketId: '' }, changes: [{ side: '', price: '', amount: '' }] } };
	}
	public render() {
		...
		return (
			<Layout>
				<div className="App">
					<header className="App-header" style={{ background: 'white', textAlign: 'center' }}>
						<h1 className="App-title">
							Welcome to DUO DEX
						</h1>
					</header>
					<p className="App-intro" style={{ textAlign: 'center' }}>
						<button onClick={() => this.sendingMessageButton()} style={{ margin: '0 0 0 20px' }}>
							Send Message
						</button>
						<button onClick={() => this.subscription()} style={{ margin: '0 0 0 20px' }}>
